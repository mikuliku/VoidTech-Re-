...
public class VoidMiningMachineMenu extends AbstractContainerMenu {
    ...
    public VoidMiningMachineMenu(int id, Inventory inv, int tier, ContainerData data,
        ...
        super(ModMenus.VOID_MINING_MACHINE.get(), id);
        ...
        for (int i = 0; i < 9; i++) {
            addSlot(new SlotItemHandler(output, i, 44 + i * 18, 40));
        }
        ...
        for (int c = 0; c < 9; c++) {
            addSlot(new Slot(inv, c, 8 + c * 18, 160));
        }
    }
    ...
    private VoidMiningMachineMenu(int id, Inventory inv, int tier, BlockPos pos) {
        this(
                ...
                new net.minecraftforge.items.ItemStackHandler(4),
                pos
        );
    }
    ...
    public ItemStack quickMoveStack(Player p, int i) {
        ...
        if (i < out) {
            if (!moveItemStackTo(st, out, slots.size(), true)) {
                return ItemStack.EMPTY;
            }
        ...
        int target =
                st.is(ModItems.SPEED_UPGRADE.get()) ? 0 :
